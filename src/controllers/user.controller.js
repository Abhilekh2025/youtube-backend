...
const registerUser = asyncHandler(async (req, res) => {
  ...
  if (!avatar) {
    throw new ApiError(400, "Avatar file is missing. Hence, required");
  }
...
const generateAccessAndRefreshTokens = async (userId) => {
  try {
    ...
    console.log("accessToken at user controller", accessToken);
...
const getHistory = asyncHandler(async (req, res) => {
  const user = await User.aggregate([
    {
      $match: {
        _id: new mongoose.Types.ObjectId("req.user._id"), //check
      },
    },
    {
      $lookup: {
